...
### 追記（2026-08-13・重要）: `db:verify` は壊れた状態を検出できていなかった

ゆまが実環境で `db:verify` → `db:setup` → `db:verify` を実行し、
すべて「✓ 定義どおりです」だった。にもかかわらず**書き込みは依然として失敗する**:

| 経路 | Appwriteの応答 |
|---|---|
| `POST /api/v1/weights` | `Unknown attribute: "needsReview"` |
| `POST /api/v1/meals` | `Unknown attribute: "fat"` |
| `POST /api/v1/migrate` | `Unknown attribute: "fat"` |

つまり **列の一覧・`status` と、書き込み経路から見えるスキーマが食い違っている。**
壊れているのは1列ではなく、**テーブルごとに別々の列**が見えなくなっている。
`weights` は先に書かれるので「成功していた」と読んでいたが、
単に移行payloadに体重が含まれていなかっただけだった（訂正）。

#### 検査方法を変えた

`status` を信じず、**本番と同じ経路で検査用の行を1件書いて確かめる**。

- Appwriteは未知の列を1つずつしか返さないため、外して再試行を繰り返し全部洗い出す
- 検査用の行は成否に関わらず必ず削除する。権限は空で作るので誰にも読めない
- `userId` は実在しない `schemaprobe` を使うので、利用者の行とは衝突しない
- `db:setup` は見つかった列を削除して作り直し、再度**書き込みで**確認する

`tests/schema-probe.test.ts` で、本番で起きた組み合わせ
（meals.fat / weights.needsReview）を再現して固定した。

#### 修復の結果（2026-08-13・確定）

書き込みで確かめる版の `db:setup` / `db:verify` を実環境で実行し、
**全9テーブルが「書き込みに使えます」**、最後も「✓ 定義どおりです」となった。

失われたデータは無い。壊れていた列があったため、
移行も通常の記録も**1行も書けていなかった**（＝消える中身が無かった）。
`rate_limits` のカウンタが作り直しでリセットされる可能性はあるが、
1日あたりの利用回数の集計にしか使っておらず実害は無い。

#### 残っている確認（人の操作が要る）

メール確認をサーバー側で必須にしたため、**確認済みアカウントでないと
書き込み経路に到達できない**。使い捨てアドレスでは確認メールを受け取れないため、
**所有者本人の実機確認が、書き込みが通ることの最終確認になる**。

現時点で言えること:

| 項目 | 状態 |
|---|---|
| スキーマが書き込みに使えること | **確認済み**（`db:setup` の書き込み検査は本番と同じ `createRow` を使う） |
| APIキーでの行作成・増分（rate_limits） | 確認済み（移行失敗時もこの段階は通過していた） |
| 本人権限つきの行の作成 | **未確認**（メール確認ゲートの先にあるため） |
| メールの実配送 | **未確認**（サーバー側でエラーは出ていないが、届いたかは受信して確かめるほかない） |

---

## 16. source of truth の切替（2026-08-13 完了）

### どう切り替えたか

保存先は**ログイン状態で自動的に決まる**。設定項目は増やしていない。

| 状態 | 正本 |
|---|---|
| ログイン済み かつ メール確認済み | **Appwrite** |
| それ以外（未ログイン・未確認） | この端末（移行前と同じ挙動） |

```
PWA ──> src/data/cloudStore（RESTを呼ぶだけ。Appwriteを知らない）
     ──> /api/v1（adapter）
     ──> api/_core/service（共通のドメイン層。将来のMCPも同じ）
     ──> repository ──> Appwrite
```

- `GET /api/v1/snapshot` … 起動時に必要なデータを一度で返す（画面ごとの往復を作らない）
- 返す形は**アプリの語彙**に統一。`$id` などAppwrite固有の項目は外へ出さない
- 一覧の上限で黙って打ち切らないよう、全件取得はカーソルで最後まで辿る

### localStorage の今の役割

| キー | 役割 |
|---|---|
| `meals` / `workouts` / `weight_history` / `user_goals` ほか記録データ | **未ログイン時の保存先**。クラウドが正本のときは**読みも書きもしない**（消さずに残すバックアップ） |
| `cache:cloud-snapshot:v1` | 起動を速くするための控え。正本ではない。ログアウトで消す |
| `reminders_enabled` / `chat_messages` | 端末に紐づくもの。移行対象外（従来どおり） |

**旧キーを消す処理はどこにも無い。** 「どちらが新しいか曖昧なら捨てない」を満たすため、
片方だけを正本にし、もう片方は触らない。

### 保存の失敗をどう扱うか

- **保存が成功してから画面を更新する。** 先に画面を書き換えて後から送る作りにしていない
- 失敗したら状態を変えず「保存できませんでした」を出す
- 書き込みは1件ずつ順に流す（二重クリックで古い状態を元に上書きしない）
- 最新を取れないときは「表示は前回の内容」と明示し、再試行を出す
- 読み込めず控えも無いときは、**空のダッシュボードを見せない**。
  画面ごと「データを読み込めませんでした／記録が消えたわけではありません」に差し替える
  （Appwrite Singapore の Databases 障害中に、この状態が実際に出ることを確認して追加した）

オフライン時の書き込みキューは作っていない。保存できないときは
はっきり失敗させる方が、消えたか保存されたか分からない状態より安全と判断した。

### 確認したこと

| 対象 | 方法 |
|---|---|
| 端末保存モードの一連（食事追加・筋トレ・種目・セット編集・リロード） | ブラウザで実機操作 |
| スナップショットの形・他人のデータが混ざらないこと | `tests/service.test.ts` |
| クラウドが正本のとき端末の記録キーに書かないこと | `tests/data-store.test.ts` |
| 保存失敗時に状態を変えないこと | 同上 |
| メール未確認ではデータAPIを使えないこと | `tests/v1-router.test.ts` + 本番 |
