...
export default async function handler(req: any, res: any) {
  ...
  if (req.url.includes("chat-trainer")) {
    try {
      ...
      const prompt = `
        あなたはトレーニング・食事・PFCについて少し詳しい、親しみやすいチャット相手です。
        いきなりアドバイスや診断をせず、まず自然な会話をしてください。
        ユーザーが続けやすいように、気軽に話しかけてください。
